...
// 这个函数的主要目的是将输入数字的字节序从大端模式转换为小端模式，或者从小端模式转换为大端模式
...
// 实现了CityHash32散列函数,用于计算字符串或字节切片的32位散列值
func CityHash32(s []byte, length uint32) uint32 {
	...
	//
	//初始化:计算前24字节的散列值
	//迭代:每次处理20字节,计算4个32位整数的散列值,更新状态变量
	//状态变量包括h、g、f,通过异或、旋转和乘法更新状态
	//迭代结束后,对状态变量进行最后处理,返回h作为最终散列值
	// length > 24
	var h uint32 = length
