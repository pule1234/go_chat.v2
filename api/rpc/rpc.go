...
func InitLogicRpcClient() {
	once.Do(func() {
		...
		//创建Logic RPC实例客户端 d：Etcd服务发现实例，用于获取逻辑RPC服务器的地址
...
func (rpc *RpcLogic) PushRoom_File(req *proto.Send) (code int, msg string) {
	reply := &proto.SuccessReply{}
	LogicRpcClient.Call(context.Background(), "PushRoom_File", req, reply)
	code = reply.Code
	msg = reply.Msg
	return
}
...
func (rpc *RpcLogic) CheckAuth(req *proto.CheckAuthRequest) (code int, userId int, userName string) {
	...
	fmt.Printf("code = %s   userid = %s,     username = %s", code, userId, userName)
...
func (rpc *RpcLogic) Push_File(req *proto.Send) (code int, msg string) {
	reply := &proto.SuccessReply{}
	LogicRpcClient.Call(context.Background(), "Push_File", req, reply)
	code = reply.Code
	msg = reply.Msg
	return
}
